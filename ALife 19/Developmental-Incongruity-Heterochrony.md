...
This first new concept is to extend the theory of heterochrony, including defining basic functions for multiphasic and compound heterochrony. Each of these types of heterochrony explains distinct phases of phenotypic complexity across developmental time, as well as the transition from one phase to the other. The mathematical development of these newly-identified forms of heterochrony are defined in Supplementary Materials (https://orthogonal-research-lab.github.io/Compound-Heterochrony.html), which provides details and mathematical formalisms, conceptual results, and theoretical scenarios for nonlinear switches during developmental growth.  

Cayley trees [21] will be used to model the expression of phenotypic change by the genotype. Each node in the tree represents a gene or regulatory element, and the tree structure itself is structured according to their relative causal effect on each other. This model demonstrates the contingent nature of single factors and their effect on collective behavior. The Cayley tree structures can be overlapped to simulate cross-talk between developmental regulatory networks. These overlapping structures not only allow us to simulate switching behaviors and nonlinear behaviors [22], but also result in emergent computational mechanisms (Figure 1). More details about the hypothetical function of overlapping trees can be found in Supplementary Materials (https://github.com/Orthogonal-Research-Lab/Developmental-Incongruities-and-Heterochrony).
